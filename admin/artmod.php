...
if(isset($_POST['modres']))
{
	...
	$sql="UPDATE `article` SET `titre` = '".$_POST['titre']."', `date` = NOW(), `categorie` = '".$_POST['catres']."', `article` = '".$_POST['res']."' WHERE `article`.`id` =".$_POST['modres']." ;";
...
<?php
	foreach ($rep as $r ) 
	{
		?>
			...
			<li><?=$r[1];?><form method="post" action="admin.php?art&mod"><button type="submit" value="<?=$r[0]?>"  name="idmod">modifier</button></form></li>
		<?php
...
{
	$linkbd = mysqli_connect("localhost","root","","transat");
	$sql="SELECT * FROM `article` WHERE id=".$_POST['idmod']."";
	$a=mysqli_query($linkbd,$sql);
	$res=mysqli_fetch_row($a);
	var_dump($res);
?>
<form method="post" action="admin.php?art&mod">
